...
class Mdl_User_Api extends CI_Model
{
	...
	function get_products($offset, $limit, $sort_by, $city, $category_id){
		...
		if($category_id != 0){
			$this->db->where('pm.product_parent_category', $category_id);
		}
		
		return $this->db->get()->result();
	}
	
	function get_packages($offset, $limit, $sort_by, $city, $category_id){
		$this->db->select('pm.Package_name, pm.packege_image, spm.*, sm.Store_name, cm.category_name');
		...
		if($category_id != 0){
			$this->db->where('pm.main_category', $category_id);
		}
	...
	function get_store_packages($offset, $limit,$user_id, $store_id){
		$this->db->select('spm.*, pm.Package_name, pm.packege_image, spm.store_id, sm.store_contact');
		$this->db->from('store_Packages_master as spm');
	...
	function get_related_products($city, $product_id, $category_id){
		$this->db->select('pm.product_name, spm.*, sm.Store_name');
		$this->db->from('store_product_master as spm');
		...
		foreach($products as $val){
			...
			$val->images = $this->db->get()->row()->image_url; 
		}
		
		return $products;
	}
	
	function get_single_pachage($user_id, $Package_id, $store_id){
		$this->db->select('pm.Package_name, pm.main_category, pm.packege_image, spm.*, sm.Store_name, sm.store_id, cm.category_name');
	...
	function get_related_packages($city, $Package_id, $category_id){
		$this->db->select('pm.Package_name, pm.packege_image, spm.*, sm.Store_name');
		$this->db->from('store_Packages_master as spm');
	...
	function get_fevourites($user_id, $type, $is_store, $offset, $limit){
		...
		if($type == 3){//store data
			...
			$this->db->where('fm.is_store', 1);
			$this->db->where('sm.store_status', 1);
			$this->db->limit($limit, $offset);
			return $this->db->get()->result();
		}else if($type == 1){//product
			$this->db->select('	fm.*, pm.product_name, spm.*, sm.Store_name ');
			$this->db->from('Favourit_item_mster as fm');
		
			$this->db->join('store_product_master AS spm','spm.product_id = fm.item_id','Left');
			$this->db->join('product_master AS pm','pm.product_id = fm.item_id','Left');
			$this->db->join('Store_master AS sm','sm.store_id = fm.store_id','Left');
			
			
			$this->db->where('fm.user_id', $user_id);
			$this->db->where('spm.store_id = fm.store_id');
			$this->db->where('sm.store_status', 1);
			$this->db->where('fm.type', 1);
			$this->db->where('fm.is_store', 0);
			$this->db->where('pm.product_status', 1);
			$this->db->where('spm.product_status', 1);
			$this->db->limit($limit, $offset);
			return $this->db->get()->result();
		}else if($type == 2){//service
			$this->db->select('	fm.*, spm.*, pm.Package_name, pm.packege_image');
			$this->db->from('Favourit_item_mster as fm');
		
		
			$this->db->join('store_Packages_master AS spm','spm.Package_id = fm.item_id','Left');
			$this->db->join('Packages_master AS pm','pm.Package_id = fm.item_id','Left');
			$this->db->join('Store_master AS sm','sm.store_id = fm.store_id','Left');
	...
	function get_Store($offset, $limit, $sort_by, $store_type, $city, $get_Store){
		$this->db->select('sm.*');
		...
		if($get_Store  != 0){
			...
			$this->db->where('scm.category_id', $get_Store);
		}
	...
	/**************** search ********************/	
	function get_search($city, $search_text, $search_type){
		
		if($search_type == 1){
			
			$this->db->select('pm.product_name as name, spm.*, sm.Store_name, tm.teg_type, tm.type');
			$this->db->from('store_product_master as spm');
			...
			$this->db->like('tm.tag', $search_text);
			//$this->db->or_like('pm.product_name', $search_text);
			...
			$this->db->select('pm.Package_name as name, pm.packege_image as images, spm.*, sm.Store_name, tm.teg_type, tm.type');
			$this->db->from('store_Packages_master as spm');
			...
			$this->db->where('sm.store_status', 1);
			$this->db->where('sm.city', $city);
			$this->db->group_by('spm.package_slug');
			
			return $this->db->get()->result();
		}else if($search_type == 3){
			$this->db->select('store_id, Store_name as name, store_image as images');
			$this->db->from('Store_master as sm');
			$this->db->group_by('sm.store_id');
			$this->db->where('sm.city', $city);
			$this->db->like('sm.Store_name', $search_text);
			$this->db->limit(15); 
			$this->db->where('sm.store_status', 1);
			return $this->db->get()->result();
		}
	...
	function get_home_products($city){
		$this->db->select('pm.product_name, spm.*, sm.Store_name, cm.category_name');
		$this->db->from('store_product_master as spm');
	...
	function get_home_packages($city){
		$this->db->select('spm.*, pm.Package_name, pm.packege_image, spm.store_id, sm.store_contact'); 
		$this->db->from('store_Packages_master as spm');
	...
	function get_home_banner(){
		...
		/**************** other ********************/
	function get_all_main_categories($city){
		$this->db->select('cm.*');
		...
		$data['product_cat'] = $this->db->get()->result();
		...
		$this->db->order_by("order", "ASC");
		$this->db->group_by("cm.category_id");
		$data['service_cat'] = $this->db->get()->result();
