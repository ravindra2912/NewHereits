<?php if($stores != NULL){ ?>
	<section class="section  pt-3 bg-white" style="margin-top: -75px;">
		<div class="container ">
			<h2 id="reviews" class="text-6 mb-3"> Stores</h2>
			<div class="row ">
				<?php foreach($stores as $res){ 
					//Store image
					$img = base_url().'assets/admin/images/no-image.png';
					if(file_exists($res->store_image)) { $img = base_url().$res->store_image; }
				
				?>
				<a href="<?= base_url()?>Store/store_details/<?= $res->store_slug ?>" class="col-md-3 col-6 p-1">
					<div class="card shadow-md border-0 mb-2" >
						<h5 class="store-name text-3 mb-0 text-black-500" style="padding: 2px 10px 2px 10px;"><?= $res->Store_name ?></h5>
						<div class="pt-2 pl-2 pr-2"><img src="<?= $img ?>" class="card-img-top d-block store-img pb-2" alt="..."></div>
			...
			<a href="<?= base_url() ?>Store" class="view-more">View More -></a>
		</div>
		
    </section>
<?php }
if($products != NULL){ ?>
	<section class="section  pt-3 bg-white" style="margin-top: -20px;">
		<div class="container ">
			<h2 id="reviews" class="text-6 mb-3"> Products</h2>
			<div class="row ">
				<?php foreach($products as $res){ 
					//product image
					$p_img = $this->Mdl_Home->get_product_single_image($res->product_id);
					$img = base_url().'assets/admin/images/no-image.png';
			...
			<a href="<?= base_url() ?>Product" class="view-more">View More -></a>
		</div>
...
<?php }
	...
	<section class="section  pt-3 bg-white" style="margin-top: -20px;">
		<div class="container ">
			...
			<a href="<?= base_url() ?>Services" class="view-more">View More -></a>
		</div>
    </section>
<?php } ?>